...
dexcalib extrinsics capture --board apriltag-1x1-75mm …  # 也可用 AprilTag 网格 / 单个 tag（board identify 可识别未知 tag）
...
leave-one-out；`--method robotcamcalib` 提供 RobotCamCalib 求解器的逐行移植作为对照（`--compare` 同时跑两种）。结果、判定标准、session 与 `results/` 格式见 [docs/handeye.md](docs/handeye.md)。
